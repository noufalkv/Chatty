...
describe('SignUp', () => {
  ...
  it('should throw an error if username is not available', async () => {
    ...
    await expect(SignUp.prototype.create(req, res)).rejects.toThrow('Invalid credentials');
  });
  ...
  it('should throw an error if username length is less than minimum length', async () => {
    ...
    jest.spyOn(cloudinaryUploads, 'uploads').mockRejectedValue(new Error('Upload failed'));
    await expect(SignUp.prototype.create(req, res)).rejects.toThrow();
  });
